    ...
    public function __invoke(Request $request): View
    {
        ...
        if ($period === 'custom' && $from?->addYears(20)->lessThan($to)) {
            throw ValidationException::withMessages([
                'to' => __('crm.campaign_report_range_too_large') ?: 'نطاق التاريخ المخصص كبير جداً. يرجى اختيار فترة لا تتجاوز 20 عاماً.',
            ]);
        }

        $campaigns = $this->visibleCampaigns($user)
            ->orderByDesc('starts_at')
            ->get(['id', 'name', 'image_path', 'starts_at', 'ends_at', 'cost']);
        ...
        $selectedStageId = isset($validated['stage_id'])
            ? (int) $validated['stage_id']
            : null;

        $conversionStage = null;
        if ($selectedStageId !== null) {
            $conversionStage = $allStages->firstWhere('id', $selectedStageId);
        }

        if ($conversionStage === null) {
            $conversionStage = $allStages->first(static fn (PipelineStage $s) => in_array($s->code, ['donor', 'contract_closed', 'closing_execution', 'execution'], true))
                ?? $allStages->where('is_active', true)->sortByDesc('position')->first()
                ?? $allStages->last();
        }

        $selectedStageId = $conversionStage?->id;

        $convertedLeads = 0;
        if ($conversionStage !== null) {
            $convertedLeads = (clone $leadQuery)
                ->whereHas('status', static function (Builder $query) use ($conversionStage) {
                    $query->where('pipeline_stage_id', $conversionStage->id);
                })
                ->count();
        }
        ...
        $metrics = [
            ...
            'conversion_rate' => $totalLeads > 0
                ...
                : 0.0,
            'conversion_stage_id' => $conversionStage?->id,
            'conversion_stage_name' => $conversionStage?->localizedName() ?? $conversionStage?->name_ar,
        ];
        ...
        return view('campaigns.reports', [
            'campaigns' => $campaigns,
            'employees' => $employees,
            'stages' => $allStages->where('is_active', true)->values(),
            'filters' => $filters,
            'metrics' => $metrics,
