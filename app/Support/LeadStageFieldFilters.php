...
final class LeadStageFieldFilters
{
    ...
    public static function resolve(
        ...
        bool $autoSelectContextStageFields = true,
        bool $pruneNonStageFields = false,
    ): array {
        $allAvailableFields = PipelineStageField::query()
        ...
        if ($contextStageId !== null && $contextStageId > 0) {
            $contextFields = $allAvailableFields->where('pipeline_stage_id', $contextStageId)->values();
            $availableFields = $contextFields;

            $hasExplicitFieldIds = $requestedFieldIds !== null && $requestedFieldIds !== '' && $requestedFieldIds !== [];
            if ($hasExplicitFieldIds) {
                ...
                $fieldIds = collect($rawFieldIds)
                    ...
                    ->unique()
                    ->values();
                $selectedFields = $pruneNonStageFields
                    ? $contextFields->whereIn('id', $fieldIds)->values()
                    : $allAvailableFields->whereIn('id', $fieldIds)->values();
            } else {
    ...
    public static function apply(Builder $query, Collection $fields, array $values): void
    {
        foreach ($fields as $field) {
            ...
            // Canonical field: check both direct leads table column and stage field values
            if ($field->binding_type === 'canonical' && ! empty($field->binding_target)) {
                $targetCol = (string) $field->binding_target;
                $query->where(static function ($subQuery) use ($field, $value, $targetCol): void {
                    $subQuery->where('leads.' . $targetCol, 'like', '%' . $value . '%')
                        ->orWhereExists(static function ($valueQuery) use ($field, $value): void {
                            $valueQuery
                                ->selectRaw('1')
                                ...
                                ->whereRaw('INSTR(LOWER(stage_field_values.value), LOWER(?)) > 0', [$value]);
                        });
                });
                continue;
            }
