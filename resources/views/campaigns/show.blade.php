...
<style>
	...
	.lead-action{min-height:40px;display:inline-flex;align-items:center;justify-content:center;gap:5px;padding:6px 12px;border:1px solid var(--line);border-radius:9px;background:#fff;color:#556174;text-decoration:none;font-size:12px;font-weight:900;cursor:pointer;transition:.18s}
	...
	.lead-more summary{width:40px;height:40px;min-width:40px;min-height:40px;display:grid;place-items:center;padding:0;border:1px solid var(--line);border-radius:9px;background:#fff;color:#596477;list-style:none;cursor:pointer;font-size:20px;font-weight:900;line-height:1}
	...
	@media(max-width:768px){
		.campaign-hero{flex-direction:column;align-items:stretch;gap:14px;padding:18px}
		.campaign-hero-copy{flex-direction:row;align-items:center;gap:12px}
		.campaign-hero-image{width:56px;height:56px;flex:0 0 56px;border-radius:14px}
		.campaign-hero-count{width:100%;min-height:70px;padding:10px}
		.campaign-hero-count strong{font-size:28px}
		.campaign-summary{grid-template-columns:1fr;padding:12px}
		.campaign-status-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
		.campaign-filter{grid-template-columns:1fr}
		.campaign-filter .btn{width:100%;min-height:44px}
		.campaign-assignment{align-items:stretch;flex-direction:column}
		.campaign-assignment .field{width:100%;max-width:none;min-width:0}
		.campaign-assignment .btn{width:100%;min-height:44px}
		.lead-action{min-height:42px;padding:8px 12px}
		.lead-more summary{width:42px;height:42px;min-width:42px;min-height:42px}
	}
	@media(max-width:380px){
		.campaign-status-grid{grid-template-columns:1fr}
	}
@endpush
		...
		<div>
			...
			<select class="campaign-live-filter" id="assigned_user_id" name="assigned_user_id">
				@foreach ($filterUsers as $filterUser)
					<option value="{{ $filterUser->id }}" @selected(! $showUnassigned && $selectedAssignee && $selectedAssignee->is($filterUser))>
