...
return new class extends Migration
{
    ...
    {
        // Find stage corresponding to no_answer (by code or containing status)
        $noAnswerStage = DB::table('pipeline_stages')->where('code', 'no_answer')->first();
        if (! $noAnswerStage) {
            $noAnswerStatus = DB::table('lead_statuses')->where('code', 'no_answer')->first();
            if ($noAnswerStatus) {
                $noAnswerStage = DB::table('pipeline_stages')->where('id', $noAnswerStatus->pipeline_stage_id)->first();
            }
        }
        ...
        // Find stage corresponding to not_interested (by code or containing status)
        $notInterestedStage = DB::table('pipeline_stages')->where('code', 'not_interested')->first();
        if (! $notInterestedStage) {
            $notInterestedStatus = DB::table('lead_statuses')->where('code', 'not_interested')->first();
            if ($notInterestedStatus) {
                $notInterestedStage = DB::table('pipeline_stages')->where('id', $notInterestedStatus->pipeline_stage_id)->first();
            }
        }
